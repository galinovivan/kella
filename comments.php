...
<?php if ( have_comments() ) : ?>
	<div class="commentlist_container">
		...
		<ol class="commentlist">
			<?php wp_list_comments('type=comment&callback=thb_comment'); ?>
		</ol>
...
<?php
	...
	$defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
		...
		'author' => '<div class="row"><div class="small-12 medium-6 columns"><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' placeholder="' . __( 'Name', THB_THEME_NAME ) . '"/></div>',
		
		'email'  => '<div class="small-12 medium-6 columns"><input id="email" name="email" type="text" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' placeholder="' . __( 'Email', THB_THEME_NAME ) . '" /></div>',
		
		'url'    => '<div class="small-12 columns"><input name="url" size="30" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" type="text" placeholder="' . __( 'Website', THB_THEME_NAME ) . '"/></div></div>' ) ),
		
		'comment_field' => '<div class="row"><div class="small-12 columns"><textarea name="comment" id="comment"' . $aria_req . ' rows="10" cols="58" placeholder="' . __( 'Your Comment', THB_THEME_NAME ) . '"></textarea></div></div>',
